...
<body>
    ...
    <?php while ($row = $examresult->fetch_assoc()) { 
        ...
        ?>
        <option value="<?php echo $row['id'];?>" <?php echo $selected;?>><?php echo $row['exam_name'];?></option>
        <?php } ?>
    </select>
  </div>
  <div class="col-md-3">
...
</form> <?php  if(isset($_GET['class_id']) && isset($_GET['exam_id']) && !isset($_GET['edit'])){ ?>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <?php foreach ($value['marks'] as $marks) { ?>
                       <td><?= $marks;?></td>
                 <?php   } ?>
                    <td><a class="btn btn-success" href="?edit=<?= $key;?>&&exam_id=<?= $_GET['exam_id'];?>&&class_id=<?= $_GET['class_id'];?>">Edit</a></td>
                </tr>
                <?php } ?>
            </tbody>
        </table>
    ...
    <?php if(isset($_GET['edit'])){ ?>
     <h3>Update Report</h3>
        ...
        <div class="col-md-3">
            ...
            <input type="text" name="mark[<?= $value['id'] ?>]" value="<?= $editdata[$key]['marks'];?>" class="form-control">
        </div>
            <?php } ?>
        <?php if (isset($editdata)): ?>
          <input type="hidden" name="subject_ids" value="<?= json_encode($subjectids); ?>">
